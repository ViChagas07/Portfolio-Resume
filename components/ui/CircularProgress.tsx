...
function useCircularProgressContext(consumerName: string) {
  ...
  if (!context) throw new Error(`\`${consumerName}\` must be used within \`${CIRCULAR_PROGRESS_NAME}\``);
  return context;
}
...
function CircularProgress(props: CircularProgressProps) {
  const {
    value: valueProp = null, getValueText = getDefaultValueText, min: minProp = 0,
    max: maxProp, size = 48, thickness = 4, label, asChild, className, children, ...rootProps
  } = props;
  ...
  const value = getIsValidValueNumber(valueProp, min, max)
    ? valueProp
    : getIsValidNumber(valueProp) && valueProp > max ? max
    : getIsValidNumber(valueProp) && valueProp < min ? min : null;
...
function CircularProgressIndicator(props: React.ComponentProps<"svg">) {
  ...
  return (
    <svg
      ...
      width={context.size} height={context.size}
      {...indicatorProps}
...
function CircularProgressTrack(props: React.ComponentProps<"circle">) {
  ...
  return (
    <circle
      data-state={context.state} cx={context.center} cy={context.center} r={context.radius}
      fill="none" stroke="currentColor" strokeWidth={context.thickness} strokeLinecap="round"
      vectorEffect="non-scaling-stroke" {...trackProps}
...
function CircularProgressRange(props: React.ComponentProps<"circle">) {
  ...
  const strokeDashoffset =
    context.state === "indeterminate" ? context.circumference * 0.75
    : context.percentage !== null ? context.circumference - context.percentage * context.circumference
  ...
  return (
    <circle
      ...
      vectorEffect="non-scaling-stroke" {...rangeProps}
      className={cn(
        "origin-center text-primary transition-colors duration-300 ease-in-out",
...
interface CircularProgressValueTextProps extends React.ComponentProps<"span"> { asChild?: boolean }
...
function CircularProgressValueText(props: CircularProgressValueTextProps) {
  ...
  return (
    <ValueTextPrimitive
      id={context.valueTextId} data-state={context.state} {...valueTextProps}
      className={cn("absolute inset-0 flex items-center justify-center font-medium text-xs", className)}
    >
...
export {
  CircularProgress, CircularProgressIndicator, CircularProgressTrack,
  CircularProgressRange, CircularProgressValueText, type CircularProgressProps,
};
